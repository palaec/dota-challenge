...
public class MatchController {
    ...
    public ResponseEntity<List<HeroKills>> getMatch(@PathVariable("matchId") Long matchId) {
    	List<HeroKills> list = matchService.findHeroAndKillsCountByMatchId(matchId);
    ...
    public ResponseEntity<List<HeroItems>> getItems(@PathVariable("matchId") Long matchId,
    	...
    	List<HeroItems> itemList = matchService.findItemAndTimestampByMatchIdAndHero(matchId,heroName);
    ...
    public ResponseEntity<List<HeroSpells>> getSpells(@PathVariable("matchId") Long matchId,
                                                      @PathVariable("heroName") String heroName) {
    	List<HeroSpells> spellList = matchService.findSpellAndSpellsCountByHeroAndMatchId(heroName,matchId);
        return new ResponseEntity<>(spellList, HttpStatus.OK);
    }

    @GetMapping("{matchId}/{heroName}/damage")
    public ResponseEntity<List<HeroDamage>> getDamage(@PathVariable("matchId") Long matchId,
                                                      @PathVariable("heroName") String heroName) {
    	List<HeroDamage> damageList = matchService.findtargetAndDamageCountByHeroAndMatchId(heroName,matchId);
